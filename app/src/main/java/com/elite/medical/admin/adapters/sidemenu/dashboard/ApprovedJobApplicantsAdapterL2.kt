...
import com.elite.medical.admin.ui.sidemenu.dashboard.jobapplicants.ApplicantDetails
...
class ApprovedJobApplicantsAdapterL2(
    private val context: Context,
    private val jobAndNurseDetails: AllJobApplicant?
) : RecyclerView.Adapter<ApprovedJobApplicantsAdapterL2.ViewHolder>() {
    ...
    override fun getItemCount() = jobAndNurseDetails!!.nurses!!.size

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {

        val item = jobAndNurseDetails!!.nurses!![position]
        ...
        holder.tv2.text = item.licenseType
        holder.tv3.text = item.licenseExpiry
        holder.tv4.text = item.experience

        holder.layout.setOnClickListener {

            val nurseDetails = jobAndNurseDetails.nurses?.get(position)
            val intent = Intent(context,ApplicantDetails::class.java)
            intent.putExtra("nurseDetails",nurseDetails)
            intent.putExtra("allDetails",jobAndNurseDetails)
