...
class ActivityNotificationsClinic: AppCompatActivity() {
    ...
    private fun populateNotifications() {
        ...
        DashboardClinicAPIs.getClinicNotifications(object : DashboardClinicAPIs.Companion.NotificationsCallback {
            override fun onListReceived(notification: List<NotificationDetailsFromAdminNotificationsModel>) {
                if(notification.isNotEmpty()){
                    val adapter =
                        NotificationAdapter(ArrayList(notification), this@ActivityNotificationsClinic)
